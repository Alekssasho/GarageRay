...
impl ShapeInterface for Sphere {
    ...
    fn intersect<'a>(
        ...
        let dpdv = (self.theta_max - self.theta_min) * Vec3::new(p_hit.z * cos_phi, p_hit.z * sin_phi, -self.radius * theta.sin());
        let d2Pduu = -self.phi_max * self.phi_max * Vec3::new(p_hit.x, p_hit.y, 0.0);
        let d2Pduv = (self.theta_max - self.theta_min) * p_hit.z * self.phi_max * Vec3::new(-sin_phi, cos_phi, 0.0);
        let d2Pdvv = -(self.theta_max - self.theta_min) * (self.theta_max - self.theta_min) * Vec3::new(p_hit.x, p_hit.y, p_hit.z);
        let E = dot(dpdu, dpdu);
        let F = dot(dpdu, dpdv);
        let G = dot(dpdv, dpdv);
        ...
        let dndu = Normal3f::from((f * F - e * g) * inv_egf2 * dpdu + (e * F - f * E) * inv_egf2 * dpdv);
        let dndv = Normal3f::from((g * F - f * G) * inv_egf2 * dpdu + (f * F - g * E) * inv_egf2 * dpdv);

        // TODO: calculate this
        let p_error = Vec3::zero();
        let isect = shape.object_to_world.transform_surface_interaction(SurfaceInteraction::<'a>::new(
            p_hit, p_error, Point2::new(u, v), -ray.d, dpdu, dpdv, dndu, dndv, ray.time, shape
        ));
